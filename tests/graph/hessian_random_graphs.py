...
def generate_four_graph(avg_us, avg_wenzel_static, avg_tapenade, avg_pytorch, denom, num_vars):
    ...
    # legend
    plt.legend(('ACORNS', 'PyTorch', 'Mitsuba', 'Tapenade'),
...
def generate_max_graph(max_us_hess, max_wenzel_hess_static, max_tapenade_hess, max_pytorch, denom):
    fig = plt.figure(figsize=(20, 10))
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(denom, max_us_hess, color='#130f40', markersize=50, linewidth=5)
    ax.plot(denom, max_pytorch, color='#ff7979', linewidth=linewidth)
    ax.plot(denom, max_wenzel_hess_static, color='#badc58', linewidth=linewidth)
    ax.plot(denom, max_tapenade_hess, color='#7ed6df', linewidth=linewidth)
    ax.set_yscale('log')
    plt.ylim(1.e-05, 1.e+03)
    plt.setp(ax.get_xticklabels(), fontsize=20)
    plt.setp(ax.get_yticklabels(), fontsize=20)
    # legend
    plt.legend(('ACORNS', 'PyTorch', 'Mitusba', 'Tapenade'),
               shadow=False, fontsize=fontsize, frameon=False)
    plt.margins(0, 0)
