...
def generate_derivatives_c_file(func_num, functions, input_filename, run_c, derivatives_filename, reverse, second_der, parallel):
    vars = ",".join(str(x) for x in functions[func_num][1])
    cmd = "python3 forward_diff.py " + input_filename + " p -ccode " + str(run_c) + " -parallel "+ str(parallel)+ " -reverse " + str(
...
def run_ours(func, num_params, functions, params_filename, output_filename, runnable_filename):
    if sys.platform.startswith('win'):
        ...
        run_command = "./" + runnable_filename + " " + \
            str(num_params) + " " + \
            str(len(func[1])) + " " + params_filename + " " + output_filename + " CXXFLAGS=-fbracket-depth=512 -O3"
    print(run_command)
...
def compile_ours(run_c, runnable_filename, utils_filename, derivatives_filename):
    if run_c:
        if sys.platform.startswith('win'):
            ...
            # cmd = "gcc -O3 -fopenmp -ffast-math -o " + runnable_filename + " " + runnable_filename + \
            #     ".c " + derivatives_filename + ".c " + " -lm"
            cmd = "/usr/local/opt/llvm/bin/clang -I/usr/local/opt/llvm/include -fopenmp -ffast-math -o " + runnable_filename + " " + runnable_filename + \
                ".c " + derivatives_filename + ".c " + " -lm -L/usr/local/opt/llvm/lib -fbracket-depth=2024"
        print(cmd)
        os.system(cmd)