...
def grad_without_traversal(ast, x=0):
    ...
    if(reverse_diff and second_der):
        c_code = c_generator.CGenerator(filename = output_filename, variable_count = len(variables), derivative_count = (len(variables)*(len(variables)+1))//2, c_code = ccode, ispc = ispc, parallel=parallel)
    elif (not reverse_diff and second_der):
        c_code = c_generator.CGenerator(filename = output_filename, variable_count = len(variables), derivative_count = (len(variables)*(len(variables))), c_code = ccode, ispc = ispc, parallel = parallel)

    else:
        c_code = c_generator.CGenerator(filename = output_filename, variable_count = len(variables), derivative_count = len(variables), c_code = ccode, ispc = ispc, parallel = parallel)
    c_code._make_header()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-parallel', type = str, default = 'False', dest = 'parallel', help='seq or parallel')
