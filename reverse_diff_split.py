...
def expand_equation(equation, dict_):
    # print("entered expand_equation: ", equation.__class__.__name__)
    if equation.__class__.__name__ == 'ArrayRef':
        # print("entered arrayref. next: ",equation.name.__class__.__name__)
        if equation.name.__class__.__name__ == 'ArrayRef':
            # print("entered arrayref in arrayref. next: ??")
            var_name = equation.name.name.name+'[{}][{}]'.format(str(equation.name.subscript.value),str(equation.subscript.value))
            # print("got var_name: ",var_name)
        else:
            # print(equation.name.name)
            # print(equation.subscript)
            if equation.subscript.__class__.__name__ == 'BinaryOp' or equation.subscript.__class__.__name__ == 'ID':
                # print('detected binary op in subscript')
                var_name = equation.name.name + '[{}]'.format(str(Expr(equation.subscript).eval()))
                # print('result:')
                # print(var_name)               
            else:
...
#     fun.show()    
    ...
    #         c_code._generate_expr([primary_base_variable._get()], v_grad, index = split_ctr, mirrored_index = None, file_pointer = file_pointer)
